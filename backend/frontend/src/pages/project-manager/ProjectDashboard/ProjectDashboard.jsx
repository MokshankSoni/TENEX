...
import { PROJECT_ENDPOINTS, AUTH_ENDPOINTS, USER_PROJECT_ASSIGNMENT, TASK_ENDPOINTS, ATTACHMENT_ENDPOINTS} from '../../../config/apiEndpoints';
...
const ProjectDashboard = () => {
  ...
  const handleAddMember = async (user) => {
    try {
      ...
      const response = await axios.post(USER_PROJECT_ASSIGNMENT.ADD_MEMBER, payload, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'X-TenantID': tenantId,
          'Content-Type': 'application/json'
  ...
  const handleConfirmRemoveMember = async () => {
    ...
    try {
      ...
      await axios.delete(USER_PROJECT_ASSIGNMENT.REMOVE_MEMBER(memberToRemove.username, memberToRemove.projectId), {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
          'X-TenantID': tenantId
          ...
          <div className="team-grid">
            {projectData.userAssignments?.slice(0, 10).map((assignment) => (
              <div key={`${assignment.username}-${assignment.projectId}`} className="team-member-card">
                ...
                {(assignment.roleInProject === 'ROLE_TEAM_MEMBER' || assignment.roleInProject === 'ROLE_CLIENT') && (
                  <button 
